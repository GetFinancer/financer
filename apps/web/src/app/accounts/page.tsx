...
export default function AccountsPage() {
  ...
  const accountAccent: Record<AccountType, { card: string; glow: string }> = {
    bank: { card: 'glass-card-blue', glow: 'glow-blue' },
    credit: { card: 'glass-card-purple', glow: 'glow-purple' },
    savings: { card: 'glass-card-green', glow: 'glow-green' },
    cash: { card: '', glow: '' },
  };
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
            {accounts.map((account) => {
              const accent = accountAccent[account.type];
              return (
                <div
                  key={account.id}
                  className={`glass-card ${accent.card} ${accent.glow} overflow-hidden relative p-6`}
                >
                  <div className="relative flex items-start justify-between mb-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        <h3 className="font-semibold flex items-center gap-2 flex-wrap">
                          {account.name}
                          {account.isDefault && (
                            <span className="text-[10px] font-medium bg-primary/20 text-primary px-2 py-0.5 rounded-md">
                              {t('accountsDefault')}
                            </span>
                          )}
                          {account.sharedUuid && (
                            <span className="text-[10px] font-medium bg-blue-500/20 text-blue-300 px-2 py-0.5 rounded-md">
                    ...
                    <span className={`text-2xl font-extrabold font-mono ${account.balance >= 0 ? 'text-income' : 'text-expense'}`}>
                      {formatCurrency(account.balance, undefined, numberLocale)}
                    </span>
                  </div>

                  <div className="relative flex gap-2 mt-2 flex-wrap">
                    <button
            ...
            <button
              ...
              className="flex items-center justify-center gap-2 min-h-[120px] rounded-2xl border-2 border-dashed border-white/20 text-muted-foreground hover:text-foreground hover:border-white/30 transition-colors text-sm"
            >
              + {t('accountsNewAccount')}
            </button>
          </div>
        )}
      </div>
